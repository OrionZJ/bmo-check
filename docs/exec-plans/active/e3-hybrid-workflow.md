...
| E3-H1 | `Classify unmatched diagnostics conservatively` | 按 `CorrelationKey`、reason、binary/scope binding 和 trace completeness 区分“本 trace 未观察到”与“身份/闭包不匹配”及“没有稳定位置”。保留 D5 registry；不新增 proof rule。 | 覆盖 closure mismatch、缺 subject、缺 PC、incomplete trace、完整且可定位但无观察等用例。前几类不得标为“未执行”；不完整 trace 不能支持缺失观察的负面结论。 |
| E3-H2 | `Expose unresolved static obligations` | 从经过 replay 的 static certificate 显式导出 `relevant_unknowns`/未闭合 obligations；扩展 snapshot 或其 typed adapter，让 D4 默认只把仍阻塞当前结论的 Unknown 当作 blocking。已 discharge 的 Unknown 可保留为历史 provenance，但必须另行标记。 | 对 SAFE+已 discharge、UNKNOWN+未 discharge 运行 contract tests；报告的 blocking ID 集必须与 certificate replay 的 unresolved/relevant set 一致。 |
| E3-H3 | `Bind dynamic certificate to trace observations` | 增加单一 typed bridge，核对 `DynamicCertificate`、manifest、trace digest、executable/libraries、DBT contract 与 snapshot；显式映射 manifest trace ID 和诊断 `TraceId`，不复制证据模型。 | Dynamic certificate 与 snapshot 来自同一 trace 时接受；证书与 snapshot trace/digest 不一致时拒绝合并。静态/动态 binary、contract 或 scope 不匹配时可分别保留各自 verdict，但 correlation 必须为 `Unmatched`/不兼容，不能报 Exact。 |
