...
- 成功的 FUTEX_WAIT/WAIT_BITSET 在 DuckDB 落盘时补成 `FUTEX_WAIT` 事件，地址是被等待的
  4 字节同步字；它保留 syscall 的 ticket 和返回序号，避免与原始 `SYSCALL_EXIT`
  共享 event id。成功的 FUTEX_WAKE/WAKE_BITSET 不访问用户字节，因此只闭合 syscall
  effect；失败等待、requeue、PI 以及未知操作不会补事件。
