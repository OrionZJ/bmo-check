...
_REPORT_SCHEMA: Final[str] = "diagnostic-report-v1"
...
class DiagnosticReport:
    ...
    def __post_init__(self) -> None:
        ...
        if any(item.scope != self.static_certificate.scope for item in unknowns):
            raise DiagnosticReportError("selected Unknown scope differs from static certificate")
...
def build_diagnostic_report(
    ...
    *,
    ...
    requested = (
        ...
        else tuple(node.id for node in all_unknowns)
    )
