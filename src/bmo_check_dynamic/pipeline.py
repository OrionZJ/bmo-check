...
def _trace_coverage(
    ...
    *,
    ...
    edges = tuple(_iter_edges(communication_edges))
    ...
    assigned_digest = _digest_edges(assigned_edges)
    communication_state = _coverage_state(
        scan_stats.complete,
        () if scan_stats.complete else ("communication edge scan is incomplete",),
    )
    window_state = _coverage_state(
        not window_unknowns,
        window_unknowns,
    )
...
def _coverage_state(complete: bool, reasons: tuple[str, ...]) -> CoverageState:
    if complete:
...
def _digest_edges(edges: tuple[CommunicationEdge, ...]) -> str:
    rows = []
    for edge in edges:
