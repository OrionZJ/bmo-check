...
from bmo_check_static.model import ModuleRole, Ordering, UnknownKind
...
def test_summary_uses_actual_lock_and_plain_store_instructions(tmp_path: Path) -> None:
    ...
    report = analyze_pthread_synchronization(
        library,
        project_root / "specs" / "static" / "pthread-api.yaml",
        project_root / "specs" / "static" / "dbt6-mo-off.yaml",
    )
